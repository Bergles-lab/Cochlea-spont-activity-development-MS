function [stats, pkData] = findICpeaksdFoF(ICsignal,filePath,analysisName,plotFlag)
    ...
    meanR = mean(pkData(:,4)); %individual mean L and R traces amplitudes
    meanL = mean(pkData(:,2));
    allEvents = cat(1,RICinfo,LICinfo);
    meanAll = mean([pkData(:,2);pkData(:,4)]) % mean peak of all data = matched peaks included twice here - Calvin added
    coVar = std([pkData(:,2);pkData(:,4)])/(meanAll) % Calvin added
    width = mean([RICinfo(:,3); LICinfo(:,3)],1)
    
%     widthRemovedRIC = [];
%     y1 = 1;
%     [length1 width1] = size(RICinfo);
%     for x1 = 1:1:length1
%         if RICinfo(x1,1) < 0.35
%             widthRemovedRIC(y1) = RICinfo(x1,3);
%             y1 = y1 + 1;
%         end
%     end
%     widthRemovedLIC = [];
%     y1 = 1;
%     [length2 width2] = size(LICinfo);
%     for x1 = 1:1:length2
%         if LICinfo(x1,1) < 0.35
%             widthRemovedLIC(y1) = LICinfo(x1,3);
%             y1 = y1 + 1;
%         end
%     end
%     
%     widthRemoved = mean([widthRemovedLIC';widthRemovedRIC'],1)
    ...
    stats = table(totLpks, totRpks, totMatchedPks, meanL, meanR, meanAll, Ldom/totPks, totPks/(length(time)/600), Ldom, Rdom, width, mean(ratio),LRcorr,coVar);
