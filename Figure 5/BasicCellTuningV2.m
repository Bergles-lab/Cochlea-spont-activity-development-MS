...
concat = [];
for i = 1:3
   concat = [concat; avgToneImg20{1,(4*(i-1)+1)} avgToneImg20{1,(4*(i-1)+2)} avgToneImg20{1,(4*(i-1)+3)} avgToneImg20{1,(4*(i-1)+4)}];
end
h=implay(concat)
...
rgb(rgb > 1) = 0.99;
rgb(rgb < 0) = 0;
v = VideoWriter('concat 6-16tuning 10db.avi','Uncompressed AVI');
open(v);
writeVideo(v,rgb);
close(v);
%% 
...
% individual movies
rgb = [];
rgb(:,:,1,:) = avgToneImg10{1,6}/255;
implay(rgb);
rgb(rgb > 1) = 0.99;
rgb(rgb < 0) = 0;
v = VideoWriter('7.7 khz 10db.avi','Uncompressed AVI');
open(v);
writeVideo(v,rgb);
close(v);
