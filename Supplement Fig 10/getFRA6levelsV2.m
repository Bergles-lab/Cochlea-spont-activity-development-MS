function [FRA,centerF,bestF,respMatrixTot,bandW,maxFall,bandWLin,bandWgaus,traceBF,traceCF,bandW70] = getFRA6levelsV2(file,numLevels,freqsFile,freqInterval)
...
% calculate maximum BW at 70 dB SPL only 
for cell = 1:file.numROI
    level = 3;
    for i = 1:length(freqsFile)
        if respMatrixTot(cell).cell(level,i) == 1
